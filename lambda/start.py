...
def lambda_handler(event, context):
    ...
    java_memory = '2G'
    spec_description = '低スペック'

    if 'queryStringParameters' in event and event['queryStringParameters']:
        params = event['queryStringParameters']
        if 'type' in params:
            req_type = params['type'].lower()
            if req_type == 'c7i':
                instance_type = 'c7i.xlarge'
                java_memory = '6G'
                spec_description = '高スペック'
    ...
    if state == 'running':
        ...
        response_script = ssm.send_command(
            ...
            Parameters={'commands': [
                f'echo "java -Xmx{java_memory} -Xms{java_memory} -jar Paper.jar" > /home/ubuntu/MinecraftGeyserServer/.docker/minecraft/cmd.sh',
                'chmod +x /home/ubuntu/MinecraftGeyserServer/.docker/minecraft/cmd.sh'
            ]},
        )
        ...
        if result['Status'] != 'Success':
            ...
            message = f'サーバーの起動が完了しました🟢\nインスタンスタイプ：{desired_type} （{spec_description}）'
